...
Post.findSingleById = function (id, visitorId = 0) {
  return new Promise(async function (resolve, reject) {
    ...
    You'll need: id
    ===============================================*/
    let [[post]] = await db.execute("SELECT p.title, p.body, p._id, p.author, p.createdDate, u.username, u.avatar FROM ournodeapp.posts p JOIN users u ON p.author = u._id WHERE p._id = ?", [id])
...
Post.findByAuthorId = async function (authorId) {
  ...
  ===============================================*/
  let [posts] = await db.execute("SELECT p.title, p.body, p._id, p.author, p.createdDate, u.username, u.avatar FROM ournodeapp.posts p JOIN users u ON p.author = u._id WHERE p.author = ? ORDER BY createdDate DESC", [authorId])
  return posts
}
...
Post.getFeed = async function (id) {
  ...
  You'll need: id
  ===============================================*/
  let [posts] = await db.execute("SELECT posts._id, posts.title, createdDate, username, avatar FROM posts JOIN users ON posts.author = users._id WHERE author IN (SELECT followedId FROM follows WHERE authorId = ?) ORDER BY createdDate DESC", [id])
