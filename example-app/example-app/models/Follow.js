...
Follow.getFollowersById = function (id) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      //authorId: Id of people who are following others
      resolve(followers)
    } catch {
      reject()
    }
...
Follow.getFollowingById = function (id) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      You'll need: id
      ===============================================*/
      let [followers] = await db.execute("SELECT username, avatar FROM follows JOIN users ON users._id = follows.followedId WHERE authorId = ?", [id])
      resolve(followers)
    } catch {
      reject()
    }
